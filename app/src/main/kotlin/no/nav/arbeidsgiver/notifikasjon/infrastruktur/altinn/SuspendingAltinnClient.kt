...
class SuspendingAltinnClient(
    private val blockingClient: AltinnrettigheterProxyKlient = defaultBlockingAltinnClient(),
    private val altinnReporteeListener: (suspend (AltinnReportee) -> Unit)? = null,
) {
    ...
    private suspend fun List<AltinnReportee>.applyListener() =
        apply {
            val listener = altinnReporteeListener ?: return@apply
            forEach {
                listener(it)
            }
        }
