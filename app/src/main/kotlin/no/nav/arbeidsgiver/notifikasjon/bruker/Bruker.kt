...
object Bruker {
    ...
    fun main(
        ...
        runBlocking(Dispatchers.Default) {
            ...
            /**
             * NB: altinn rolle eksisterer kun i dev. fjern før merge
             */
            launchProcessingLoop(
                "cleanup duplicate mottakere altinn reportee",
                pauseAfterEach = Duration.ofSeconds(2)
            ) {
                val db = database.await()
                ...
                val resultat = db.nonTransactionalExecuteBatch("""
                    delete from mottaker_altinn_reportee
                    where id = ?
                """,
                    duplikateIder.flatMap { it.sorted().drop(1) }.take(10_000)
                ) {
                ...
                if (duplikateIder.isNotEmpty()) {
                    log.info("fant ${duplikateIder.size} altinn-reportee-mottakere med duplikater. slettet ${resultat.sum()} rader av ${resultat.size} forsøkte")
                }
            }
             ...
             */
            launchProcessingLoop(
                "cleanup duplicate mottakere altinn rolle",
                pauseAfterEach = Duration.ofSeconds(2)
            ) {
                val db = database.await()
                ...
                val resultat = db.nonTransactionalExecuteBatch("""
                    delete from mottaker_altinn_rolle
                    where id = ?
                """,
                    duplikateIder.flatMap { it.sorted().drop(1) }.take(10_000)
                ) {
                ...
                if (duplikateIder.isNotEmpty()) {
                    log.info("fant ${duplikateIder.size} altinn-rolle-mottakere med duplikater. slettet ${resultat.sum()} rader av ${resultat.size} forsøkte")
                }
            }
        }
    }
