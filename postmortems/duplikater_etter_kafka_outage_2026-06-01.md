...
|---|--------|---------------|-------------|
| 3 | Server-side idempotens-record | [idempotent oppretting](../specifications/idempotent_oppretting_notifikasjon.md) | Persistér og gjenbruk den tilfeldige id-en per `(merkelapp, eksternId)`, slik at en retry produserer *samme* id og konsumenten dedup-er på primærnøkkel. To alternativer utredet: **reservation** (mindre) og **transaksjonell outbox** (anbefalt — fjerner produce-timeout fra request-pathen). `hendelseId`-generering endres ikke. Dette er årsaksfiksen; #2 er en symptomfiks (duplikatene produseres fortsatt). |
