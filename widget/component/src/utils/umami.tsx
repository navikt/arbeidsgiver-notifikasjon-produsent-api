import { useConsent } from '../hooks/ConsentContext';
...
const mockedUmami = (): {
  ...
  track: (eventInput: string, eventProperties?: Record<string, any>) => {
    console.group('Mocked umami-event');
    ...
    return {
      promise: new Promise<Types.Result>((resolve) =>
        resolve({
          ...
          code: 200,
          message: 'Success: mocked umami-tracking',
        }),
      ),
    };
  },
...
export const UmamiProvider = ({ miljo, children }: { miljo: Miljø, children: ReactNode }) => {
  ...
  useEffect(() => {
    if (trackingId === undefined) {
      setUmami(mockedUmami());
    } else if (consent && consent.analytics) {
      const script = initializeUmamiScript();
      setUmami(window.notifikasjonWidgetUmami);
      return () => {
